...
export class ParticlesDirective implements OnDestroy, OnInit {
  ...
  @HostListener("mousemove", ["$event"]) onMouseMove(e: { offsetX: any; offsetY: any; }) {
    this.setMousePos(e.offsetX, e.offsetY);
  }

  @HostListener("touchmove", ["$event"]) onTouchMove(e: { touches: { clientY: any; }[]; }) {
...
class Particle {
  ...
  quad!: QuadTree;
  explored!: boolean;
  constructor (canvas: HTMLCanvasElement, r: number | undefined) {
      this.r = r;
      this.speedScale = particleSpeed / 2;
      this.reset(canvas, r);
  }
  reset(canvas: HTMLCanvasElement, r = this.r) {
  ...
  addPath(ctx: { moveTo: (arg0: number, arg1: number) => void; arc: (arg0: number, arg1: number, arg2: number, arg3: number, arg4: number) => void; }) {
      ctx.moveTo(this.x + this.r,  this.y);
        ctx.arc(this.x,  this.y, this.r, 0, TAU);
  }
  near(p: { x: number; y: number; }) {
      return ((p.x - this.x) ** 2 + (p.y - this.y) ** 2) <= linkDistance2;
  }
  intersects(range: { x: number; y: number; w: any; h: any; }) {
  ...
  update(ctx: CanvasRenderingContext2D, repulse = true) { 
      this.explored = false;
      const r = this.r;
      let W, H;
...
class Bounds {
  ...
  bottom: number;
  diagonal: number;
  constructor(x: number, y: number, w: number, h: number) { this.init(x, y, w, h) }
...
class QuadTree {
  ...
  addToSubQuad(particle: { quad: undefined; }) {
  ...
  query(part: Particle, fc: number, found: Particle[]) {
